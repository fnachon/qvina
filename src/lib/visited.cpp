...
double ele::dist2_3D(const std::vector<double>& now) const {
	double d, out=0;
	for(int i = 0; i < 3; ++i) {
		d = x[i] - now[i];
		out += d * d;
...
//	bool effect=false;
	...
	const sz nowDSize = now_d.size();
	for (sz i = 0; i < nowDSize; ++i) {
		bitMask = ONE << i;
...
int Octree::interesting(conf& x, double f, change& g, int excluded) {
	...
	conf_v.reserve(g.num_floats());
	x.getV(conf_v);
	std::vector<double> change_v;
	change_v.reserve(g.num_floats());
	g.getV(change_v);
	std::vector<ele> nearbyPoints;
	nearbyPoints.reserve(conf_v.size());
	std::vector<double> distances;
	distances.reserve(conf_v.size());
	...
	const int len = static_cast<int>(nearbyPoints.size());
	bool notYetChecked[len];
	memset(notYetChecked, true, sizeof(notYetChecked));

	const int grandMaxCheck = static_cast<int>(conf_v.size()); // 1N in this case
	const int maxCheck = (static_cast<int>(nearbyPoints.size()) <= grandMaxCheck) ? static_cast<int>(nearbyPoints.size()) : grandMaxCheck;

	double min = 1e10;
	int i = 0; // counts checks done so far
